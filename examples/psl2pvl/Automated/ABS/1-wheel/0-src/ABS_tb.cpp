...
/* psl 
vunit main(Main){
  assert always active(s.send) && speed_s.written() -> within_t[(1,SC_MS)] active(ecu_absasr.read_s) && speed_s.read() && ecu_absasr.v[0] == s.send.speed;
  assert never waiting(s.send) && 
  ...
  && ecu_absasr.read_s.wait_event().not_notified()  // Does not exist but is wrongly assessed
